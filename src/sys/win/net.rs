...
// use std::net::{TcpSocket, UdpSocket, SocketAddr, TcpSocket};
...
unsafe fn slice2buf(slice: &[u8]) -> WSABUF {
    println!(
        "read len = {:?}",
        cmp::min(slice.len(), <u_long>::max_value() as usize) as u_long
    );
    WSABUF {
...
#[cfg(test)]
mod tests {
    use std::net::{TcpSocket, UdpSocket, TcpSocket, SocketAddr};
    use std::thread;
    use std::io::prelude::*;

    use Overlapped;
    use iocp::CompletionPort;
    use net::{TcpSocketExt, UdpSocketExt, SocketAddrBuf};
    use net::{TcpSocketExt, TcpSocketExt, AcceptAddrsBuf};
    use net2::TcpSocket;
    ...
    fn tcp_connect() {
        each_ip(&mut |addr_template| {
            let l = t!(TcpSocket::bind(addr_template));
            let addr = t!(l.local_addr());
            let t = thread::spawn(move || {
                t!(l.accept());
            });
            ...
            let builder = match addr {
                SocketAddr::V4(..) => t!(TcpSocket::new_v4()),
                SocketAddr::V6(..) => t!(TcpSocket::new_v6()),
            };
            t!(cp.add_socket(1, &builder));

            let a = Overlapped::zero();
            t!(builder.bind(addr_template));
            let (s, _) = unsafe { t!(builder.connect_overlapped(&addr, &[], a.raw())) };
            let status = t!(cp.get(None));
    ...
    fn tcp_accept() {
        each_ip(&mut |addr_template| {
            ...
            let addr = t!(l.local_addr());
            let t = thread::spawn(move || {
                let socket = t!(TcpSocket::connect(addr));
            ...
            let (s, _) = unsafe { t!(l.accept_overlapped(&builder, &mut addrs, a.raw())) };
            let status = t!(cp.get(None));
            assert_eq!(status.bytes_transferred(), 0);
            assert_eq!(status.token(), 1);
            assert_eq!(status.overlapped(), a.raw());
